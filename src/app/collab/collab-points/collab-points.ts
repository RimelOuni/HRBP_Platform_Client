// collab-points.component.ts — VERSION COMPLÈTE CORRIGÉE
...
@Component({
  selector:      'app-collab-points',
  standalone:    true,
  imports:       [CommonModule, FormsModule],
  templateUrl:   './collab-points.html',
  styleUrls:     ['./collab-points.css'],
  encapsulation: ViewEncapsulation.None,
})
export class CollabPoints implements OnInit {
  ...
  load(): void {
    this.loading = true;
    this.error   = '';
    this.svc.getMyPoints().subscribe({
      next: pts => {
        this.points  = pts;
        this.loading = false;
        this.applyFilters();
  ...
  loadReclams(): void {
    ...
    this.svc.getMyReclamations().subscribe({
      next: r  => {
  ...
  // ✅ Tri générique : PENDING d'abord (date desc), puis PROCESSED (date desc), puis REJECTED (date desc)
  ...
  // ── Getters pour le template (séparation des groupes) ────────────
  ...
  resetFilters(): void {
    ...
    this.critFilter   = 'Tous';
    this.freqFilter   = 'Tous';
    this.searchQuery  = '';
    this.dateFrom     = '';
    this.sortOrder    = 'desc';
    this.applyFilters();
  ...
  statusCls(s?: string): string {
    return ({ 'En attente': 'cp-s-pending', 'En cours': 'cp-s-progress', 'Terminé': 'cp-s-done', 'Annulé': 'cp-s-cancelled' } as any)[s || ''] || 'cp-s-pending';
  }

  msgCls(s?: string): string {
    return ({ PENDING: 'cp-b-pending', PROCESSED: 'cp-b-done', REJECTED: 'cp-b-rejected' } as any)[s || ''] || 'cp-b-pending';
  }
  ...
  critCls(c?: string): string {
    return ({ Basse: 'cp-crit-low', Moyenne: 'cp-crit-med', Haute: 'cp-crit-high' } as any)[c || ''] || '';
  }
  ...
  submitReq(): void {
    ...
    this.savingReq = true;
    this.svc.createPointRequest(this.reqForm.titre, this.reqForm.commentaire, this.reqForm.date_souhaitee).subscribe({
      next: r => {
  ...
  submitRec(): void {
    ...
    this.savingRec = true;
    this.svc.createReclamation(this.recPoint._id, this.recForm.titre, this.recForm.commentaire, this.recForm.nouvelle_date_proposee || null).subscribe({
      next: r => {
